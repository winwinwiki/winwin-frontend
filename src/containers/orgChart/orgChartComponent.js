...
export class OrgChartComponent {
    ...
    renderOrgChart() {
        ...
        let matrixX = (((this.orgData.children.length + 1) * 250) - orgContainerwidth) / 2;
        let options = {
            data: this.orgData,
            direction: 't2b',
            ...
            nodeTemplate: (node) => {
                if (node.id.toString().indexOf('_add') > -1) {
                    return `<div class="title p-2"></div><div class="content"><p class="text-center">${node.name}</p></div>`;
                }
                return `<div class="title p-2">${node.name}</div>
                    <div class="content">
                        ...
                        <div class="row m-0">
                            <div class="col-sm-12 px-2">${node.children ? (node.children.length - 1) : 0} ${node.childrenType}s</div>
                            ${node.relationship !== '001' ? `<div class="col-sm-12 px-2 text-right viewOrgDetails"><a href="javascript:;">View Details</a></div>` : ''}
                        </div>
                    </div>
                </div>`;
            }
        ...
        $('#orgChart .orgchart').css("transform", 'matrix(1, 0, 0, 1,-' + matrixX + ' ,0)');
        $('#orgChart .orgchart .node').click((e) => {
            let orgNode = $(e.target).closest('.node');
            let nodeId = $(e.target).closest('.node').attr('id');
            if ((nodeId.toString().indexOf('_add') == -1) && nodeId != _self.orgData.id) {
                ...
                this.addNewChild(orgNode.attr('data-parent'), nodeId.toString().replace(/(.)*_add/ig,''));
            }
            $('#orgChart .orgchart').css("transform", 'matrix(1, 0, 0, 1, 0 ,0) scale(1,1)');
        });
