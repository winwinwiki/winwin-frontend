...
class SideBar extends React.Component{
    ...
    componentWillReceiveProps() {
        ...
        if(pathname.indexOf('programs')>-1 || pathname.indexOf('new-program') > -1){
        ...
        return (
            <div id="mySidenav" className="sidenav d-flex flex-column">
                <ul className="list-group list-group-flush pr-3">
                    ...
                    <li className="list-group-item"><Link className={activeNav === ''?'active':''} onClick={()=>{this.changeActiveNav(''); this.setState({activeSubNav:''})}} to={`${match.url}`}>Basic Information</Link></li>
                    <li className="list-group-item"><Link className={activeNav === 'organization-chart'?'active':''} onClick={()=>{this.changeActiveNav('organization-chart'); this.setState({activeSubNav:''})}} to={`${match.url}/organization-chart`}>Organization Chart</Link></li>
                    <li className="list-group-item">
                        <a href="javascript:;" className={activeNav === `${type}-details`?'active':''} onClick={()=>{this.changeActiveNav(`${this.props.type}-details`); this.setState({activeSubNav:''})}} data-target="#collapseExample" data-toggle="collapse" aria-expanded="false" aria-controls="collapseExample">Organisation Details</a>
                    ...
                    <li className="list-group-item"><Link className={activeNav === 'programs'?'active':''} to={`${match.url}/programs`} onClick={()=>{this.changeActiveNav(`programs`); this.setState({activeSubNav:''})}}>Programs</Link></li>
                    <li className="list-group-item"><Link className={activeNav === 'notes'?'active':''} to={`${match.url}/notes`} onClick={()=>{this.changeActiveNav(`notes`); this.setState({activeSubNav:''})}}>Notes</Link></li>
                </ul>
    ...
    renderSubNavOptions(){
        ...
        return subNavOptions.map(option => <li onClick={()=>this.setState({activeSubNav:option.path})} key={option.path}><Link className={activeSubNav === option.path?'active':''} to={`${match.url}/${option.path}`}><i></i>{option.title}</Link></li>)
    }
